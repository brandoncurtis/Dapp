...
//$ meteor deploy dapp.meteorapp.com --settings settings.json
...
var oracleAddress = "0x1f71046028c203ad21b41d2e1c44c3bf31e73ddc";
...
var oABI = [{"constant":true,"inputs":[{"name":"","type":"bytes32"}],"name":"documentStructs","outputs":[{"name":"name","type":"bytes32"},{"name":"value","type":"uint256"}],"payable":false,"type":"function","stateMutability":"view"},{"constant":false,"inputs":[{"name":"x","type":"bytes32"}],"name":"bytes32ToString","outputs":[{"name":"","type":"string"}],"payable":false,"type":"function","stateMutability":"nonpayable"},{"constant":false,"inputs":[{"name":"key","type":"bytes32"},{"name":"name","type":"bytes32"},{"name":"value","type":"uint256"}],"name":"StoreDocument","outputs":[],"payable":false,"type":"function","stateMutability":"nonpayable"},{"inputs":[],"payable":false,"type":"constructor","stateMutability":"nonpayable"},{"anonymous":false,"inputs":[{"indexed":false,"name":"_name","type":"string"},{"indexed":false,"name":"_value","type":"uint256"}],"name":"Print","type":"event"}];
...
Template.bulletin.events({
	...
	'click button.bulletin': function (err, template) {
	 ...
	 var check = "Address / Notional / Long Margin / Short Margin / Long Premium / Short Premium / Start Date / End Date";
	let transferEvent = fContract.Print({}, {fromBlock: 0, toBlock: 'latest'})
	transferEvent.get((error, logs) => {
	  // we have the logs, now print them
	  logs.forEach(log => console.log(log.args['_value']))
	  	...
	  	for(i = 0; i < logs.length; i++){
			...
			if (j>0 && j<4) {console.log('State- ', sInstance.currentState.call().toNumber())
				var startDate = eval(web3.toAscii(sInstance.startDate.call()).substring(0,8)); 
				if (startDate > date ){
					var notional = web3.fromWei(sInstance.notional.call().toNumber(), 'ether');
					var lmargin = web3.fromWei(sInstance.lmargin.call().toNumber(), 'ether');
					var smargin = web3.fromWei(sInstance.smargin.call().toNumber(), 'ether');
					var l_premium = web3.fromWei(sInstance.l_premium.call().toNumber(), 'ether');
					var s_premium = web3.fromWei(sInstance.s_premium.call().toNumber(), 'ether');;
					var endDate = eval(web3.toAscii(sInstance.endDate.call()).substring(0,8)) ;
					 check +='<br>'+ add + "/ "+ notional+"/ "+lmargin+"/ "+smargin +"/ " + l_premium+"/ "+s_premium+"/ "+startDate+"/ "+endDate;
				}
			}
		}
		}
...
Template.Oracle.events({
	...
	var fdate = web3.fromAscii(date);
	console.log(fdate);
	var oInstance = web4.eth.contract(oABI).at(oracleAddress);
	var j = oInstance.documentStructs.call(fdate)[1] / 1000;
	console.log(j);
	document.getElementById("oraclevals").innerHTML = j;
}
});

Template.mySwaps.events({

	'click button.OpenSwaps': function (err, template) {
	 var check = "Address / Current State / Long Party / Short Party / Notional / Long Margin / Short Margin / Long Premium / Short Premium / Start Date / End Date";
	let transferEvent = fContract.Print({}, {fromBlock: 0, toBlock: 'latest'})
	transferEvent.get((error, logs) => {
	  // we have the logs, now print them
	  logs.forEach(log => console.log(log.args['_value']))
	  	for(i = 0; i < logs.length; i++){
			...
			if (sInstance.notional.call().toNumber() >0){
			var j = sInstance.currentState.call().toNumber();
			if (j<4) {console.log('State- ', sInstance.currentState.call().toNumber())
				var startDate = eval(web3.toAscii(sInstance.startDate.call()).substring(0,8)); 
					...
					var s_premium = web3.fromWei(sInstance.s_premium.call().toNumber(), 'ether');;
					var endDate = eval(web3.toAscii(sInstance.endDate.call()).substring(0,8)) ;
					 check +='<br>'+ add + "/ "+j + "/ " + long_party + "/ " + short_party + "/ "+ notional+"/ "+lmargin+"/ "+smargin +"/ " + l_premium+"/ "+s_premium+"/ "+startDate+"/ "+endDate;
			}
		}
		}
	...
	'click button.ClosedSwaps': function (err, template) {
	 var check = "Address / Current State / Long Party / Short Party / Notional / Long Margin / Short Margin / Long Premium / Short Premium / Start Date / End Date";
	let transferEvent = fContract.Print({}, {fromBlock: 0, toBlock: 'latest'})
	transferEvent.get((error, logs) => {
	  // we have the logs, now print them
	  logs.forEach(log => console.log(log.args['_value']))
		...
		document.getElementById("closedswaplist").innerHTML = check;
	})
}
});
...
Template.swap.events({
    ...
    console.log('Addres length',s_address.length)
    console.log('lsad',s_address)
    var flag = 0;
    	if (s_address.length != 42) {flag = 1; document.getElementById("addresserr").innerHTML = "Error- Enter valid address";}
    	if (bools[1] != 2) {flag = 1;document.getElementById("ecperr").innerHTML = "Error - You must be an ECP to perform swap (e.g. true)";}
    	if (nums[2] <= 0 ){flag= 1;document.getElementById("usermarerr").innerHTML = "Error - Enter a margin > 0";}
    	if (nums[3] <=0){flag = 1;document.getElementById("cptyerr").innerHTML = "Error- Enter a margin > 0";}
    	if(nums[4] <=0 ){flag =1;document.getElementById("notionalerr").innerHTML = "Error- Enter a notional > 0";}
    	if (bools[5] != (true||false)){flag = 1;document.getElementById("longerr").innerHTML = "Error- Must be true or false";}
    	if (parseInt(x.elements[6]) > 20170000 && parseInt(x.elements[6]) < 20200000){flag = 1;document.getElementById("startdateerr").innerHTML = "Error- Enter a valid date in the form YYYYMMDD e.g. 20171025";}
    	if (parseInt(x.elements[7]) > 20170000 && parseInt(x.elements[7]) < 20200000){flag = 1;document.getElementById("enddateerr").innerHTML = "Error- Enter a valid date in the form YYYYMMDD e.g. 20171025";}
    	if (nums[8] < 0 ){flag= 1;document.getElementById("longpremerr").innerHTML = "Error - Enter a premium >= 0";}
    	if (nums[9] <0){flag = 1;document.getElementById("shortpremerr").innerHTML = "Error- Enter a premium >= 0";}
